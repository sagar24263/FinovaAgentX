"""
Fund Service — fund search, performance, breakups, comparison.
Reference: investmentagent/app/services/fund_service.py
"""
...
def _insurer_matches(query: str, candidate: str) -> bool:
    q = query.lower().strip()
...
def _format_fund(fund: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if fund.get("BenchMarkFundName"):
        result["benchmark"] = {
            "Name": fund.get("BenchMarkFundName"),
    ...
    if not insurer_name:
        ...
        if len(insurers) > 1:
            return json.dumps({
                ...
                "message": f"Available from: {', '.join(sorted(insurers))}. Which insurer?",
            })

    return json.dumps(_format_fund(matches[0]), default=str)
...
async def get_plan_fund_performance(plan_name: str, insurer_name: Optional[str] = None, top_n: int = 5) -> str:
    ...
    top_funds = [
        {"FundName": f.get("FundName"), "5Y": f.get("FiveYrGrowth"), "7Y": f.get("Ret7YR"), "10Y": f.get("Ret10YR")}
...
async def get_plan_funds_split_by_type(plan_name: str, insurer_name: Optional[str] = None, top_n: int = 5) -> str:
    ...
    return json.dumps({
        ...
        "passive_funds": [],
        "note": "Passive classification requires NFO timeline integration."
    }, default=str)
...
async def compare_funds(fund_name_1: str, fund_name_2: str, insurer_name_1: Optional[str] = None, insurer_name_2: Optional[str] = None) -> str:
    ...
    return json.dumps({"fund_1": _format_fund(matches1[0]), "fund_2": _format_fund(matches2[0])}, default=str)
